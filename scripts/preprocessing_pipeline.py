...
@hydra.main(
    config_path="../conf",
    config_name="preprocessing_tcga_samples",
    version_base=None,
)
def pipeline(cfg: DictConfig):
    ...
    for dirclass in lst_dirs:
        ...
        cmd = (
            ...
            f" --preset {preset} --use_simple_seg {use_simple_seg} --stitch"
        )
        print(cmd)
        cmd_list = cmd.split()
        ...
        csv_path = os.path.join(savedirclass, "slides_format.csv")
        cmd = (
        ...
        df = pd.read_csv(os.path.join(savedirclass, "slides_format.csv"))
        df["case_id"] = df["slide_id"]
        df["label"] = classname
        lst_df.append(df)

    df_final = pd.concat(lst_df, ignore_index=True)
    df_final.to_csv(os.path.join(feat_dir, "slides_format.csv"), index=False)
